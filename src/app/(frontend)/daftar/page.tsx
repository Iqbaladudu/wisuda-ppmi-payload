...
export default async function DaftarPage() {
  ...
  // Fetch registration status via API to decouple from direct Payload call
  const baseUrl = process.env.BASE_URL || ''
  const res = await fetch(`${baseUrl}/api/registration-status`, { next: { revalidate: 15 } })
  let registrationStatus: { effectiveOpen: boolean; statusMessage: string; manual?: any }
  if (res.ok) {
    const data = await res.json()
    registrationStatus = {
      effectiveOpen: data.status?.effectiveOpen,
    ...
    registrationStatus = {
      effectiveOpen: false,
      statusMessage: 'Tidak dapat memuat status pendaftaran saat ini.',
    }
  }
    ...
    <div className="relative isolate w-full min-h-screen overflow-hidden pt-36 pb-24 text-white before:absolute before:inset-0 before:-z-30 before:bg-[#140A08] after:absolute after:inset-0 after:-z-20 after:bg-[radial-gradient(circle_at_40%_30%,rgba(230,140,90,0.18),transparent_70%)]">
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-5 md:px-8 lg:flex-row lg:items-start lg:gap-12">
        <div className="w-full">
          ...
          {registrationStatus.effectiveOpen ? (
            <div className="relative z-10 w-full p-4 md:p-6 lg:p-8">
              <MultiStepForm />
            </div>
          ) : (
            <RegistrationClosed message={registrationStatus.statusMessage} />
          )}
        </div>
      </div>
    </div>
