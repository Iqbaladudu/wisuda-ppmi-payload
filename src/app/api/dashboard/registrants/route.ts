...
export async function GET() {
  try {
    ...
    // Get all registrants dengan limit tinggi untuk akurasi maksimal
    const registrantsResult = await payload.find({
      collection: 'registrants',
      depth: 0,
      limit: 10000, // Maksimal untuk akurasi
    })
    ...
    // Process each registrant untuk akurasi maksimal
    ...
    // Helper function untuk format breakdown dengan persentase akurat
    const formatBreakdown = (
      ...
      return Object.entries(data)
        .map(([key, count]) => ({
          ...
          count,
          // Persentase dengan 2 desimal untuk akurasi maksimal
    ...
    // Format all breakdowns dengan persentase akurat
    const breakdowns = {
    ...
    Object.keys(breakdowns).forEach(key => {
      breakdowns[key as keyof typeof breakdowns] = breakdowns[key as keyof typeof breakdowns].map((item, index) => ({
        ...item,
        rank: index + 1,
      }))
    })

    // Calculate advanced metrics dengan akurasi tinggi
    ...
    const insights = {
      ...
      diversityIndex: {
        major: Object.keys(counters.major).length,
        faculty: Object.keys(counters.faculty).length,
        university: Object.keys(counters.university).length,
        kekeluargaan: Object.keys(counters.kekeluargaan).length,
      }
    }
    ...
    return NextResponse.json({
      ...
      overview: {
        ...
        dataFreshness: {
          recordsProcessed: registrantsResult.docs.length,
          totalRecords: totalRegistrants,
          processingComplete: registrantsResult.docs.length === totalRegistrants,
        }
      },
      breakdowns,
      insights,
