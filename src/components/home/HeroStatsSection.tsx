...
interface HeroStat { label: string; value: number; suffix?: string }

// Lightweight CountUp again (duplicated minimal to avoid importing whole HeroAlternative logic)
const CountUp: React.FC<{ target: number; suffix?: string; delay?: number; duration?: number }> = ({ target, suffix = '', delay = 0.15, duration = 1.4 }) => {
  ...
  return <span ref={ref} className="tabular-nums font-bold tracking-tight">{display}{suffix}</span>
}
...
export const HeroStatsSection: React.FC<HeroStatsSectionProps> = ({ stats, progressPercent, currentRegistrants, targetRegistrants }) => {
  return (
        ...
        {stats.map((stat) => (
          ...
          >
            ...
            <div className="relative flex flex-col gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-white/55">{stat.label}</span>
              <span className="text-2xl md:text-3xl font-bold tracking-tight tabular-nums"><CountUp target={stat.value} />{stat.suffix}</span>
            </div>
      ...
      >
        ...
        <div className="relative flex flex-col gap-7">
          <div className="flex flex-wrap items-center gap-4 justify-between">
            <span className="text-[11px] uppercase font-semibold tracking-wider text-white/65">Indikator Dinamis</span>
        ...
        <motion.div
          ...
          style={{ mask: 'linear-gradient(black,black) content-box, linear-gradient(black, black)', WebkitMaskComposite: 'xor' as any }}
        />
      </motion.div>
    </div>
  );
